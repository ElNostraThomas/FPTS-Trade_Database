...
2. **[2/5]** `sync-fp.py` — pulls fresh data from the Fantasy Points API (6 endpoints) + Sleeper API (players + stats + projections) → rewrites `data/values.json`, `data/articles.json`, `data/rank-history.json`, `data/projections.json`.
...
| `data/projections.json` | Weekly + season projections (auto-generated by `sync-fp.py`) |
