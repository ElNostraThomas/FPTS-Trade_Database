...
1. **Admin scratchpad stale-CSV data-loss risk (HIGH)** — when `tiers.html` is loaded from `file://` on a local desktop folder that's behind `origin/main`, the page reads the stale local `tiers.csv` and Publish writes that stale state back up, clobbering any newer admin edits committed from other sessions (e.g. another browser tab, a previous publish, or a manual edit). Loveland's row ping-ponged across `d3bb5c7 → a98d0cf → 1e16d75` because of this — metadata like `up,buying,Top Target,Dynasty` got added in session A and silently reverted in session B from a stale page. Fix options: (a) Publish does a GET-latest from GitHub first and merges, (b) tiers.html always loads via GitHub Pages URL (not file://), (c) Publish refuses if the page's CSV version timestamp is older than what's on origin.
2. **values-supplement layer for force-include players** — `data/values.json` is built by walking FP's `/v2/adp/dynasty/nfl` response (`sync-fp.py:514-515`), so a player only ends up there if FP returned them. As of 2026-05-22 sync, Deebo Samuel, Tyreek Hill, and Eli Stowers are absent — likely FP filters veteran FAs from dynasty ADP. Tier rows for them render with no value/team/posRank. Fix: new `data/source/values-supplement.csv` that `sync-fp.py` reads and merges into `values.json` after the FP fetch, with manually-curated rank/age/team/pos. ~30 lines of code.
3. **Reorder UX for tiers** — current ▲/▼ arrows on the section headers require N clicks to move a tier N slots. If section reordering becomes frequent, add drag-and-drop here too (mirror the Phase 4 row pattern).
4. **Bulk tier rename** — single-tier rename only today. Probably unnecessary, but flag-worthy.
5. **Publish dry-run / diff preview** — Publish is "fire one PUT per file." A pre-flight that shows the operator the upcoming diff before committing would harden the workflow (and would also surface the stale-CSV issue in #1).
