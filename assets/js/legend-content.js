    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: '2025 Season Stats · Full PPR row group', what: 'Position-aware rows aggregating STATS_DATA[name:norm].seasons[2026]. Renders all 12 stat rows but HIDES any row where every player has 0 — keeps the table tight for cross-position comparisons.', source: '_pcTableSeasonGroup(year) + _pcSeasonStatRows() (single source of truth for which stats and which mode). Scoring via _pcStdFpts(s) — see FORMULAS.md §50.', values: 'Best: max for everything except INT (min). FPTS computed live from raw stats via _pcStdFpts so the column matches PPR + 4-pt pass TD baseline.', notes: 'Cross-position comparisons (e.g. RB vs WR) hide rows where both players have 0 — so comparing 4 WRs won\'t show empty passing rows.' },
          ...
          { label: 'Best-in-row highlighting', what: 'Per-cell green tint + green-border ::before overlay on the winner. Skips highlight when all valid cells are equal (tie) or only one cell is valid. See FORMULAS.md §47 for the helper.', source: '_pcBestIdx(values, mode) in compare.html — returns the cell index that wins, or -1 when nothing should be highlighted.', values: 'mode = "max" or "min" per row definition.', notes: 'Null/undefined cells excluded from the comparison so an empty slot doesn\'t accidentally become the "winner."' },
        ],
      },
    ],
  },
