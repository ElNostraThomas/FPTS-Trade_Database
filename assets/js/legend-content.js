  ...
  'prospectmodel': {
    ...
    sections: [
      ...
      },
      {
        name: 'Context blocks',
        items: [
          { label: 'Usage Share (donut + share bars)', what: 'The prospect\'s FINAL college season, shown with the same donut and share bars the Player Profile page uses for an NFL season.', source: 'The last row of prospects.json seasons[] — raw PFF college data; renderUsage() in prospect-model.js', values: 'RB donut is Touch mix (carries vs receptions) — deliberately the same split the profile page shows for an NFL back. WR/TE donut is Target mix (downfield vs screens). Share bars: RB breakaway rate / yards after contact per carry / PFF rushing grade; WR+TE catch rate / contested catch rate / drop rate.', notes: 'Screens are split out because the models carry BOTH a with-screens and a minus-screens career efficiency number, so how much of a receiver\'s target diet was screens is part of reading his grade. Alignment (wide/slot/inline) is a STACKED BAR, not a donut, because the donut recipe this page inherits is documented "two categories only" and alignment is three; run scheme (zone vs gap) is exactly two, so it stays a donut. Alignment rates rarely sum to 100 — rounding plus unclassified snaps — so the bar prints "N% classified" and leaves the remainder as empty track rather than inflating the slices. The two non-percentage share bars (yards after contact, PFF grade) print their real units — the track position is scaled, the number is not.' },
          { label: 'vs NFL column + Model likes / NFL likes sorts', what: 'On the class board: how far the model\'s opinion of a prospect sits from the NFL\'s, in percentile points, plus two sorts that rank the whole class by that disagreement.', source: 'deltaFor() in prospect-model.js — model percentile minus draft-capital percentile, both already in the payload', values: 'Positive = the model likes him more than his draft slot does; negative = the NFL did. 2026 extremes: Eli Heidenreich +40, Eric McAlister +36, Tanner Koziol +34 on the model side; Kaden Wetjen −37, Zavion Thomas −36 on the NFL side.', notes: 'Computed here rather than read from the workbooks\' own Delta column. WR and TE compute Delta against their FINAL model percentile (exact on 346/346 and 163/163 rows), but RB computes DC Delta against the WITHOUT-BEST-SEASON percentile (233/233) — not the variant the card leads with — so shipping the raw columns would silently mean different things per position. Computing it here also makes it follow the variant switcher. A prospect with no draft capital has no delta and sorts LAST in both directions: no opinion is not the same as agreement.' },
          { label: 'Prospect grade in the player drawer', what: 'The shared drawer on every page shows the prospect grade beside its "Prospect model" link, so the number is readable without leaving the page you are on.', source: 'player-panel.js reads data/prospect-index.json — key -> [pos, class, grade, pct], 26 KB, fetched once on first drawer open and never on page load', values: 'The grade shown is the position\'s DEFAULT variant, since the drawer has no variant switcher and must show what the card leads with.', notes: 'A chip rather than a table column on purpose. Measured coverage: 79% of the tradeable universe (mvs.json) at WR/RB/TE, 61% of the ADP startup board, 53% of the full player dictionary, and 0% of QBs — the model is a curated prospect list with an inclusion threshold, not a census. A column would be blank a fifth to a half of the time; a chip simply does not appear. Styled quiet deliberately: a college grade is history for a player with several NFL seasons.' },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Tabs Row (.pp-tabs)', what: 'Tab switcher for the bottom pane: Trades, Player Stats, Age Curve, Trade Finder, ADP Heatmap, ADP Trend.', source: 'ppShowTab() in assets/js/player-panel.js', values: '6 tabs; last-selected persisted in ppLastTab variable so reopening the panel restores it', notes: 'ppLastTab is cleared on closePanel() so a fresh open starts on Trades. SYNC-RULE: the same strip must appear on every player modal — adp-tool.html keeps a parallel inline copy via _adpShowTab().' },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Asset-type filter', what: 'Toggle QB / RB / WR / TE and draft picks (per year) to control which asset types the suggested packages may use. Strict: pick WR and packages are WR-only; pick nothing and it considers everything (the default).', source: 'trade-finder.js mlTfAssetAllowed filters the pool inside mlTfPickOffers; per-year buttons from mlTfAllPickYears', values: 'Applies to the package side — what you SEND in FOR, what you GET in AWAY. Empty selection = unchanged.', notes: 'Removes the perceived randomness — you decide what it offers. See Formulas §5 (trade-finder).' },
          { label: 'Suggestion variety', what: 'The few offers shown each anchor on a DIFFERENT primary asset where possible — so the list isn\'t the same stud with a swapped 3rd/4th piece. Only falls back to repeating an anchor when there aren\'t enough distinct ones to fill the list.', source: 'trade-finder.js mlTfPickVariety — primary = the package\'s highest-value asset; prefers fresh primaries, mirrors the engine\'s usedPrimaries guard', values: 'Applies to both Fair (fairest-first) and Aggressive (range-spread) modes; also drives the calc Trade Builder.', notes: 'Fixes clustering from merging the fit + value engine passes. See Formulas §3 (trade-finder).' },
          { label: 'Trade Builder uses the same engine', what: 'The one-at-a-time Trade Builder (the ✓/✗ pre-screener you open from a player or pick) now runs through the exact same quality pipeline as the sidebar finder — fair-mode build target, anti-lowball, startability, the owner-willingness banner, and the value-to-your-team lineup-fit lens — instead of raw value-matched packages.', source: 'assets/js/trade-calc.js mltbBuildOffers() → window.TradeFinder.pickOffers / startThresholds / ownerSell / lineupFit / rankByFit', values: 'Same curator-tunable constants as the finder; falls back to the raw engine if a league context is unavailable.', notes: 'The target is valued in the league format (SF / 1QB / TEP) so the edge math matches the asset pool. See Formulas §6 (trade-finder).' },
        ],
      },
      {
