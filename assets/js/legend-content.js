    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Tab: Player Stats (#pp-stats-tab)', what: 'Sleeper-backed per-season stat table, position-aware columns. Each year row is CLICKABLE — a small chevron (▸ collapsed, ▾ expanded) toggles an inline expansion showing every week that year (regular season + playoffs). Playoff weeks are marked with a bright orange "PLAYOFF" chip after the week number.', source: 'renderPlayerStats() in assets/js/player-panel.js — fetches https://api.sleeper.com/stats/nfl/player/{sleeperId}?season={yr}&season_type=regular&grouping=season for each year. Weekly breakdown fires on click via _fetchWeeklyStats(sleeperId, year) which hits the same endpoint with grouping=week + a parallel season_type=post call. Per-(player,year) result cached in _weeklyStatsCache for the page session.', values: 'Position-specific column sets — QB: FPTS/GP/Pass Yds/Pass TD/INT/Cmp%/Att/Rush Yds/Rush TD/PPG. RB: FPTS/GP/Rush Yds/Rush Att/Rush TD/YPC/Rec/Rec Yds/Rec TD/PPG. WR + TE: similar receiving slice.', notes: 'Async-guard pattern per CLAUDE.md §1: every fetch captures the target player name at call time and bails at resolution if the user has swapped to a different compared player. The drawer\'s inline-style table-row constants (thStyle, thStyleName, tdStyle, tdNum, tdName) are reused for the weekly child rows so the cell layout aligns vertically with the season row above. NOTE: this tab still uses Sleeper /stats for the per-year drill-down; data suite is also available as window.STATS_DATA[key].seasons[year] (used by my-leagues + live-draft projection pipeline). A future cleanup can migrate this drawer to STATS_DATA for consistency.' },
